    ...
    <form id="customerForm" action="{{ route('admin.customers.store') }}" method="POST">
      ...
      <div class="row mb-3">
        <div class="col-md-6">
          ...
          <input type="text" class="form-control @error('line') is-invalid @enderror" name="line" value="{{ old('line') }}">
          @error('line')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
      ...
      <div class="row mb-3">
        <div class="col-md-6">
          ...
          <input type="text" class="form-control @error('subdistrict_name') is-invalid @enderror" name="subdistrict_name" id="subdistrict_search" value="{{ old('subdistrict_name') }}" placeholder="Masukan Minimal 4 karakter untuk mencari" required>
          <input type="hidden" name="subdistrict_id" id="subdistrict_id" value="{{ old('subdistrict_id') }}">
        ...
        <div class="col-md-6">
          ...
          <select class="form-select @error('customer_category_id') is-invalid @enderror" name="customer_category_id" id="customer_category_id" required>
            ...
            @foreach($categories as $category)
              <option value="{{ $category->id }}" {{ old('customer_category_id') == $category->id ? 'selected' : '' }}>{{ $category->category_name }}</option>
            @endforeach
          </select>
      ...
      <div class="row mb-3">
        ...
        <div class="col-md-6">
          ...
          <select class="form-select @error('is_active') is-invalid @enderror" name="is_active">
            ...
            <option value="N" {{ old('is_active') == 'N' ? 'selected' : '' }}>Nonaktif</option>
          </select>
          @error('is_active')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
      ...
      <input type="hidden" name="is_private_order" value="0">
...
<script>
  $(document).ready(function() {
    ...
    // If subdistrict_id is already set (e.g. on form validation error), load postal codes
